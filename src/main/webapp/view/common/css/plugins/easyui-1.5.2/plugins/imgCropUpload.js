;(function(global, $, Crop) {
    ...
    */
    ...
    function _createDom($wrap, opt) {
        ...
        var $cropDataInp = $('<input type="hidden" name="cropData" id="cropData_'+opt.cropParam.out+'">');
        var $picker = $('<div class="crop-picker-wrap"><button class="crop-picker" type="button">选择图片</button></div>');
        var $fileInp = $('<input type="file" id="file_'+opt.cropParam.out+'" name="file" accept="' + accept + '" class="crop-picker-file">');
    ...
    function _bind($cropObj, opt) {
        ...
        $save.click(function(eve) {
            eve.preventDefault();
            Crop.upload(opt.cropParam.out, opt.url);
        });
    ...
    function upload(out,url) {
		...
		form.action=form.action+"?x="+JSON.parse($("#cropData_"+out).val()).x+
		"&y="+JSON.parse($("#cropData_"+out).val()).y+"&h="+JSON.parse($("#cropData_"+out).val()).h+
		"&w="+JSON.parse($("#cropData_"+out).val()).w;
//        form.submit();
		$.ajaxFileUpload({  
            url:form.action,  
            secureuri:false,  
            fileElementId:"file_"+out,//file标签的id  
            ...
            success: function (data) {
            	if(data.status!=0){
    		    	...
    		    	$('#'+out).val(data.status);
    		    	try {
    		    		eval($("#"+Crop.crop.id).attr("onselect"))
    		    	} catch(e) {}
            	}
